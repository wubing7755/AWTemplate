      ...
      handles.forEach((handle) => {
        ...
        if (direction === "bottom" || direction === "top") {
          ...
          if (!panel || panel.getBoundingClientRect().height === 0) {
            panel = handle.parentElement?.querySelector(
              '.bottom-panel, [class*="bottom"], [id*="bottom"]',
            ) as HTMLElement;
          }
        }
        ...
        const minSize =
          handle.dataset.minSize || panel.dataset.minSize
            ? Math.max(
                38,
                Number.parseInt(
                  handle.dataset.minSize || panel.dataset.minSize || "38",
                  10,
                ),
              )
            : 38;
        ...
        _instances.push(
          new ResizablePanel(panel, handle, direction, { minSize }),
        );
      });
